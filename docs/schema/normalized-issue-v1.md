...
branch_name, workspace_path, base_ref, base_ref_config, and base_sha are top-level compatibility aliases and are null until a workspace row exists
git mirrors the git-related workspace fields for prompt ergonomics and is null until a workspace row exists
base_ref is the resolved Git ref; base_ref_config preserves the configured value such as auto
...
| `issue.base_ref` | `issue.workspace.base_ref` / `git.base_ref` | Upstream-style issue context with resolved ref. |
| `issue.base_ref_config` | `issue.workspace.base_ref_config` / `git.base_ref_config` | Diagnostics for `auto` vs explicit config. |
...
| branch_name, workspace_path, base_ref, base_ref_config, base_sha | aliases from `workspaces` |

...
| Handoff state | A successful run can end in workflow-defined handoff state, not necessarily `Done`. | `handoff.submit` only records handoff data. The run finalizer must generate a review packet; only then issue state becomes `Human Review`. | Compatible extension | `docs/implementation/IS-004-cli-tool-gateway.md`, `docs/implementation/IS-010-review-packet-artifacts.md` |
...
When upstream SPEC and Local v1 differ, implement Local v1 behavior only when this matrix marks the difference as an intentional extension or deviation. Otherwise treat the mismatch as a documentation bug and update this matrix before implementation.

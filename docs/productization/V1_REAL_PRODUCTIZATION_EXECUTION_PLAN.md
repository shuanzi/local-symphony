...
- [x] B3 / 安全策略执行与回归套件：已完成；默认 command/network/protected-path policy evaluator 接入 Codex approval bridge，auto-deny 写入 `approval_requests(auto_denied)` 并返回 canonical failure code。

...
**阶段状态**：**v1.1 WIP** —— D1 主体 + R1 修复 + R2 review 跑中（2 P2 forwarding）；D3 / R14 5 轮 codex review 0 finding 收口；D5 / R13 主体 + R1 修复 + R2 修复 commit 落地（R2 review 文档待生成）；D2 / D4 准备中；D6 / R15 文档合同收口（本批）。
...
| R10 | Review Packet API | D1 | 🟡 D1 进行中 | `2cc1888` + R1 `0aee74c` | 主体 + R1 修复落地；**R2 review 跑中**：2 P2 forwarding，见 §3 已知限制 |
...
### 3.1 D1 / R10 — R2 review 跑中（2 P2）

D1 R1 修复 commit `0aee74c` 修完 R1 的 3 个 finding 后，当前 D6 tree 保留 2 个仍需 D1 owner 处理的 P2 finding。已验证 `schemas/review_packet.schema.json` 顶层 `required` 不包含 `artifacts`，因此不再把 top-level `artifacts` 作为 blocker 转交。
...
2. P2 #2 修法：把 `internal/httpapi/httpapi.go:866` 的 `raw_secret_exposed: false` 改为 `a.Kind == "secret_artifact" || a.Kind == "secrets"`，与 local store 对齐。

D6 范围内**不修**，仅记录。

### 3.2 D5 / R13 — R2 review 文档待生成

D5 / R13 release packaging 是跨 PR 跟踪项；当前 D6 tree 不包含 release script 或 web lockfile，因此本文不声明 `scripts/build-release.sh` / `npm ci` 行为。`docs/productization/D5_CODEX_REVIEW_ROUND2.md` 文档仍待 D5 实施 agent 生成。

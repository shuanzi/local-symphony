...
**阶段状态**：**v1.1 WIP** —— D1 主体 + R1 修复 + R2 review 跑中（1 P1 + 2 P2 forwarding）；D3 / R14 5 轮 codex review 0 finding 收口；D5 / R13 主体 + R1 修复 + R2 修复 commit 落地（R2 review 文档待生成）；D2 / D4 准备中；D6 / R15 文档合同收口（本批）。
...
| R9 | CLI over REST + daemon session | C4 | 🟡 C4 v1.1 WIP | `83e5e44` … `b9af4a6` | 12 commits（5 轮 codex + 6 轮 adversarial + 1 docs）；trust 边界专项 4 项见 §3 已知限制 |
| R10 | Review Packet API | D1 | 🟡 D1 进行中 | `2cc1888` + R1 `0aee74c` | 主体 + R1 修复落地；**R2 review 跑中**：1 P1 + 2 P2 forwarding，见 §3 已知限制 |
...
| `docs/codex/FIXTURE_POLICY.md` | M | CI 段补 D3 / R14 preflight + D5 / R13 `npm ci` 状态 |
...
### 3.1 D1 / R10 — R2 review 跑中（1 P1 + 2 P2）

D1 R1 修复 commit `0aee74c` 修完 R1 的 3 个 finding 后，codex review round 2 暴露 3 个新 finding（详见 `docs/productization/D1_CODEX_REVIEW_ROUND2.md`）：

- **P1**：`schemas/review_packet.schema.json` 顶层 `required` 加了 `artifacts`，但 `Generator.Generate` 永远不写顶层 `artifacts`，导致新生成的 `review.json` 不能通过 file schema 校验。
...
- **P2 #2**：`raw_secret_exposed` 在 daemon 路径恒为 `false`，在 local store 路径按 `kind` 判断；两条路径返回同一 `secret_artifact` 时 dashboard 会闪烁。

**forwarding to D1 实施 agent**：

1. P1 修法（择一）：从 `schemas/review_packet.schema.json` 顶层 `required` 移除 `artifacts`（HTTP 路径从 DB 构造，不需要受 file schema 约束）；或让 `Generator.Generate` 写顶层 `artifacts`。
2. P2 #1 修法：用 union of two enums，至少补 `prompt_snapshot / codex_log / secret_artifact / secrets / agent_file / diagnostic`。
3. P2 #2 修法：把 `internal/httpapi/httpapi.go:866` 的 `raw_secret_exposed: false` 改为 `a.Kind == "secret_artifact" || a.Kind == "secrets"`，与 local store 对齐。

D6 范围内**不修**，仅记录。

### 3.2 D5 / R13 — R2 review 文档待生成

D5 R2 修复 commit `cdf08ed`（F1-r2 默认 `OUT_DIR=$ROOT/dist` 允许 / F2-r2 追踪 `web/package-lock.json` / F3-r2 永远跑 `npm ci`）已落地，但 `docs/productization/D5_CODEX_REVIEW_ROUND2.md` 文档待 D5 实施 agent 生成。
